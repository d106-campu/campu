...
CREATE TABLE `campsite_original` (
  ...
  `tel` varchar(32) DEFAULT NULL COMMENT '전화',
...
CREATE TABLE `campsite` (
  ...
  `homepage` varchar(1024) DEFAULT NULL COMMENT '홈페이지',
  `sited_stnc` int(11) DEFAULT NULL COMMENT '사이트간 거리',
  `map_image_url` varchar(1024) DEFAULT NULL COMMENT '배치도',
  `animal_cmg_cl` varchar(16) DEFAULT NULL COMMENT '애완동물출입',
...
-- d106.campsite_induty definition

CREATE TABLE `campsite_induty` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '식별번호',
  `campsite_id` bigint(20) NOT NULL COMMENT '캠핑장 식별번호',
  `induty_id` bigint(20) NOT NULL COMMENT '유형 식별번호',
  PRIMARY KEY (`id`),
  KEY `campsite_induty_induty_FK` (`induty_id`),
  KEY `campsite_induty_campsite_FK` (`campsite_id`),
  CONSTRAINT `campsite_induty_campsite_FK` FOREIGN KEY (`campsite_id`) REFERENCES `campsite` (`id`),
  CONSTRAINT `campsite_induty_induty_FK` FOREIGN KEY (`induty_id`) REFERENCES `induty` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='캠핑장 유형';
...
CREATE TABLE `room` (
  ...
  `type` varchar(8) NOT NULL COMMENT '방 타입',
