...
export const ChangePhoneModal = ({
  ...
  const handleChange = (field: keyof IUserProfileUpdate, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if ((field === 'tel' || field === 'verifyNums') && (value === '' || /^\d+$/.test(value))) {
          ...
          if (res.data.available) {
            ...
            sendVerificationCode.mutate(values.tel, {
              ...
              onError: (error) => {
                ...
                setErrors(prev => ({ ...prev, tel: '잠시 후에 다시 시도해주세요.' }));
              }
            });
          } else {
            setErrors(prev => ({ ...prev, tel: '이미 등록된 휴대폰 번호입니다.' }));
            setIsPhoneAvailable(false); // 중복 검사 통과 못함
          }
        },
        onError: (error) => {
          console.error('휴대폰 번호 중복 확인 에러:', error);
          setErrors(prev => ({ ...prev, tel: '휴대폰 인증을 실패했습니다. 다시 확인해주세요.' }));
        }
      });
    } else {
      ...
      verifyPhone.mutate(requestData, {
        onSuccess: (response) => {
          ...
          if (isVerified) {
            ...
            updatePhoneMutation.mutate({ tel: values.tel }, {
              onSuccess: () => {
        ...
        onError: (error) => {
          ...
          setErrors(prev => ({ ...prev, verifyNums: '인증 과정에서 오류가 발생했습니다.' }));
        }
      });
    } else {
      setErrors(prev => ({ ...prev, verifyNums: '인증번호는 6자리 숫자여야 합니다.' }));
    }
  };
