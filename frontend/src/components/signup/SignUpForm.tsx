...
ILoginFormProps): JSX.Element => {
  ...
  const handleSubmit = () => {
    ...
    const allFieldsValid = Object.values(errors).every((error) =>
      [
        "사용 가능한 아이디입니다.",
        "사용 가능한 닉네임입니다.",
        "올바른 비밀번호가 설정되었습니다.",
    ...
    if (values.phone.length === PHONE_LENGTH) {
      ...
      checkPhone.mutate(values.phone, {
        ...
        onError: (error) => {
          ...
          setErrors((prev) => ({
            ...prev,
            phone: "휴대폰 인증을 실패했습니다. 다시 확인해주세요.",
          }));
        },
      });
    } else {
  ...
  useEffect(() => {
    Object.keys(values).forEach((field) => {
      validateField(
        field as keyof ISignUpFormValues,
        values[field as keyof ISignUpFormValues]
      );
    });
  }, [phoneVerified]);
        ...
        <div className="w-[100%] h-auto flex items-center justify-center rounded-2xl shadow-2xl bg-white relative">
          <div className="w-[80%]">
            ...
            <div className="h-[30px] pt-8 flex items-center justify-center">
              <p className="text-center font-bold text-xl">회원가입</p>
            </div>
            <p
              className={
                isSmallScreen
                  ? "cursor-pointer visible text-centerfont-bold text-xs absolute top-3 right-10"
                  : "invisible"
              }
              onClick={toggleForm}
            >
